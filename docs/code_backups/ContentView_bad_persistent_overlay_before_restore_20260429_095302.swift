...
final class AppState: ObservableObject {
    ...
    private var persistentImageAnchorOverlayNode: SCNNode?
    ...
    func sendCurrentFrameToSolver() {
        ...
        persistentImageAnchorOverlayNode?.removeFromParentNode()
        persistentImageAnchorOverlayNode = nil
    ...
    private func startDynamicImageAnchorTracking(response: SolveResponse, capturedImage: UIImage) {
        ...
        let referenceImage = ARReferenceImage(
            referenceCGImage,
            orientation: .up,
            physicalWidth: boardPhysicalWidthMeters
        )
    ...
    func attachImageAnchorOverlay(to anchorNode: SCNNode, imageAnchor: ARImageAnchor) {
        ...
        // If ARKit temporarily loses image tracking, do NOT remove or hide the overlay.
        // Keep the last known good world-space pose so the digits do not disappear
        // when the user moves fast or briefly looks away.
        if !imageAnchor.isTracked {
            hasActiveSolutionOverlay = true
            statusText = "Solved | holding last board pose"
            return
        }

        let wrapper: SCNNode

        if let existing = persistentImageAnchorOverlayNode {
            wrapper = existing
        } else {
            // Remove fallback table-plane overlay once true image-anchor placement is available.
            worldSolutionNode?.removeFromParentNode()
            worldSolutionNode = nil

            wrapper = SCNNode()
            wrapper.name = "persistentImageAnchorSolutionOverlay"
            ...
            // Slightly transparent so the printed grid remains visible beneath it.
            material.transparency = 0.88

            plane.materials = [material]

            let overlayPlane = SCNNode(geometry: plane)
            overlayPlane.name = "solutionTexturePlane"

            // ARImageAnchor's local image lies in the X/Z plane.
            // SCNPlane lies in local X/Y by default, so rotate it flat.
            overlayPlane.eulerAngles.x = -.pi / 2.0

            // Tiny offset above paper to avoid z-fighting without looking floaty.
            overlayPlane.position = SCNVector3(0, 0.00002, 0)

            wrapper.addChildNode(overlayPlane)
            sceneView.scene.rootNode.addChildNode(wrapper)
            persistentImageAnchorOverlayNode = wrapper
        }

        // Update persistent node to the latest actively tracked image-anchor pose.
        // When tracking is lost later, we simply keep this last transform.
        wrapper.simdTransform = imageAnchor.transform
