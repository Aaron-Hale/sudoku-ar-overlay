...
final class AppState: ObservableObject {
    ...
    func sendCurrentFrameToSolver() {
        ...
        resetDynamicImageTrackingForNewSolve()
    ...
    private func resetDynamicImageTrackingForNewSolve() {
        ...
        // Clear old ARImageAnchors so a new puzzle does not keep using
        // the previous board reference image.
        ...
        // Remove old anchors without fully resetting world tracking.
        sceneView.session.run(config, options: [.removeExistingAnchors])
    }

    func imageAnchorTrackingLost() {
        // Prefer approximate but stable fallback over a disappearing overlay.
        worldSolutionNode?.isHidden = false
        hasActiveSolutionOverlay = true
        statusText = "Solved | image tracking lost, using fallback"
    }
    ...
    func attachImageAnchorOverlay(to anchorNode: SCNNode, imageAnchor: ARImageAnchor) {
        ...
        // If image tracking is currently weak/lost, hide the image-anchor overlay
        // and reveal the fallback table-plane overlay instead.
        ...
        // Image anchor is actively tracked, so prefer it.
        ...
        if let existing = anchorNode.childNode(withName: overlayName, recursively: false) {
            ...
            // Slight transparency so printed grid remains visible.
