...
final class AppState: ObservableObject {
    ...
    private func makeWorldSolutionNode(
        ...
        let textScale = min(width, height) / 27.0
        ...
        for r in 0..<9 {
            for c in 0..<9 {
                ...
                if given == 0 && value != 0 {
                    ...
                    digit.position = SCNVector3(x, 0.00005, z)
...
struct ContentView: View {
    ...
    var body: some View {
        ZStack(alignment: .top) {
            ...
            // Minimal status only before solve / while solving / on failure.
            // After a successful solve, the screen should show only the AR digits.
            if appState.isSolving || appState.lastSolveResponse?.status != "solved" {
                VStack(spacing: 6) {
