...
final class AppState: ObservableObject {
    ...
    // Image-anchor pose is used only to initialize a persistent world anchor.
    private var lockedBoardWorldAnchorID: UUID?
    private var lastImageAnchorTransform: simd_float4x4?
    private var imageAnchorStableFrameCount: Int = 0
    private let imageAnchorStableFrameThreshold: Int = 8
    private let imageAnchorStableTranslationMeters: Float = 0.006
    private let imageAnchorStableAxisDotThreshold: Float = 0.995
    ...
    func sendCurrentFrameToSolver() {
        ...
        resetDynamicImageAnchorStateForNewSolve()
    ...
    private func resetDynamicImageAnchorStateForNewSolve() {
        guard let sceneView else {
            return
        }

        imageAnchorSolutionTexture = nil
        lockedBoardWorldAnchorID = nil
        lastImageAnchorTransform = nil
        imageAnchorStableFrameCount = 0

        // Clear old image/world anchors so a new puzzle does not reuse the old one.
    ...
    func attachLockedWorldAnchorOverlay(to anchorNode: SCNNode) {
        guard let solutionTexture = imageAnchorSolutionTexture else {
            return
        }

        let overlayName = "lockedWorldSolutionOverlay"

        if anchorNode.childNode(withName: overlayName, recursively: false) != nil {
            return
        }

        // Once world anchor is active, remove the fallback table-plane overlay.
        worldSolutionNode?.removeFromParentNode()
        worldSolutionNode = nil
        ...
        // ARImageAnchor/world anchor transform uses the image plane orientation.
        // SCNPlane is local X/Y, so rotate it flat into X/Z.
        overlayNode.eulerAngles.x = -.pi / 2.0

        // Nearly flush. Keep tiny offset to avoid z-fighting.
        overlayNode.position = SCNVector3(0, 0.00002, 0)

        anchorNode.addChildNode(overlayNode)

        hasActiveSolutionOverlay = true
        statusText = "Solved | board world anchor locked"
    }

    func attachImageAnchorOverlay(to anchorNode: SCNNode, imageAnchor: ARImageAnchor) {
        // Important: the image anchor is no longer the render anchor.
        // It is only a pose detector used to create a persistent AR world anchor.

        guard imageAnchorSolutionTexture != nil else {
            return
        }

        guard lockedBoardWorldAnchorID == nil else {
            return
        }

        guard imageAnchor.isTracked else {
            statusText = "Solved | waiting for image tracking..."
            return
        }

        let transform = imageAnchor.transform

        if let last = lastImageAnchorTransform {
            ...
            let translationDelta = simd_length(p1 - p0)

            let lastX = simd_normalize(SIMD3<Float>(last.columns.0.x, last.columns.0.y, last.columns.0.z))
            let newX = simd_normalize(SIMD3<Float>(transform.columns.0.x, transform.columns.0.y, transform.columns.0.z))

            let lastZ = simd_normalize(SIMD3<Float>(last.columns.2.x, last.columns.2.y, last.columns.2.z))
            let newZ = simd_normalize(SIMD3<Float>(transform.columns.2.x, transform.columns.2.y, transform.columns.2.z))

            let xDot = simd_dot(lastX, newX)
            let zDot = simd_dot(lastZ, newZ)

            if translationDelta < imageAnchorStableTranslationMeters &&
                xDot > imageAnchorStableAxisDotThreshold &&
                zDot > imageAnchorStableAxisDotThreshold {
                imageAnchorStableFrameCount += 1
            } else {
                imageAnchorStableFrameCount = 1
            }
        } else {
        ...
        lastImageAnchorTransform = transform

        if imageAnchorStableFrameCount < imageAnchorStableFrameThreshold {
            statusText = "Solved | locking board pose \(imageAnchorStableFrameCount)/\(imageAnchorStableFrameThreshold)"
            return
        }
        ...
        let lockedAnchor = ARAnchor(name: "lockedSudokuBoardWorldAnchor", transform: transform)
        lockedBoardWorldAnchorID = lockedAnchor.identifier

        sceneView.session.add(anchor: lockedAnchor)

        statusText = "Solved | board world anchor requested"
    }
...
struct ARSudokuView: UIViewRepresentable {
    ...
    final class Coordinator: NSObject, ARSCNViewDelegate {
        ...
        func renderer(_ renderer: SCNSceneRenderer, didAdd node: SCNNode, for anchor: ARAnchor) {
            if anchor.name == "lockedSudokuBoardWorldAnchor" {
                Task { @MainActor in
                    self.appState?.attachLockedWorldAnchorOverlay(to: node)
                }
                return
            }
