...
final class AppState: ObservableObject {
    ...
    func sendCurrentFrameToSolver(tapPoint: CGPoint? = nil) {
        ...
        let resizedSnapshot = resizeImagePreservingAspect(snapshot, maxLongSide: 1600)

        guard let jpeg = resizedSnapshot.jpegData(compressionQuality: 0.88) else {
            statusText = "Could not create AR view snapshot JPEG."
            return
        }

        isSolving = true
        statusText = "Frame/table locked. Sending resized snapshot to solver..."
    ...
    private func resizeImagePreservingAspect(_ image: UIImage, maxLongSide: CGFloat) -> UIImage {
        let size = image.size
