...
final class AppState: ObservableObject {
    ...
    private func makeSolutionTexture(givens: [[Int]], solution: [[Int]]) -> UIImage {
        ...
        return renderer.image { _ in
            ...
            let font = UIFont.systemFont(ofSize: 78, weight: .semibold)
            ...
            // Calibration: draw the 9x9 digit layout slightly inside the full board texture.
            // This compensates when detected board corners include a small margin around
            // the actual printed Sudoku grid.
            let insetX: CGFloat = canvasSide * 0.030
            let insetY: CGFloat = canvasSide * 0.030
            let usableWidth: CGFloat = canvasSide - 2.0 * insetX
            let usableHeight: CGFloat = canvasSide - 2.0 * insetY
            let cellW: CGFloat = usableWidth / 9.0
            let cellH: CGFloat = usableHeight / 9.0

            // Optional tiny shifts for quick calibration.
            let shiftX: CGFloat = 0.0
            let shiftY: CGFloat = 0.0
            ...
            for r in 0..<9 {
                for c in 0..<9 {
                    ...
                    let x = insetX + CGFloat(c) * cellW + shiftX
                    let y = insetY + CGFloat(r) * cellH + shiftY

                    let rect = CGRect(
                        x: x,
                        y: y + (cellH - textSize.height) / 2.0 - 4.0,
                        width: cellW,
